...
def query_classname(request):
    try:
        if(request.method == "POST"):
            ...
            if (request.session.get("id")):
                name_query=python_functions.regex_list_to_string(
                python_functions.validate_query_entries(request.POST.get("name_query")))
                if(name_query):
                    list_classes=python_functions.academic_users_search_classes_by_classname(request=request,classname=name_query)
                    if(list_classes):
                        context={
                            'list_classes':list_classes,
                            'query_active':True,
                            'query_classname':name_query
                        }
...
def query_professorname(request):
    try:
        if(request.method ==  "POST" and request.session.get("id")):
                ...
                name_query=python_functions.regex_list_to_string(
                python_functions.validate_name_entries(request.POST.get("name_query")))
                if(name_query):
                    list_classes=python_functions.academic_users_search_classes_by_professorname(request=request,professorname=name_query)
                    context={
                        'list_classes':list_classes,
                        'query_active':True,
                        'query_classname':name_query
    ...
    try:
        if(request.method ==  "POST"):
            if(request.session.get("RA")):
                ...
                python_functions.validate_query_entries(request.POST.get("name_query")))
                if(name_query):
                    list_classes=python_functions.students_search_classes_by_classname(request=request,classname=name_query)
                    context={
                        'list_classes':list_classes,
                        'query_active':True,
                        'query_classname':name_query
